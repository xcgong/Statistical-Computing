...
simul1<-function(N,lambda,Nsim=1000,seeds=NULL){
  ...
  colnames(result)=c("N","¦Ë","Bias.u","Bias.c","Bias.m","MSE/N.u","MSE/N.c","MSE/N.m")
...
simul2<-function(N,lambda,Nsim=1000,seeds=NULL){
  ...
  colnames(result)<-c("¦Ë",rep(as.character(N),2))
  return(as.data.frame(result))
}
...
table3<-cbind(table3.temp,'¦Ò2'=table2[7:14,2])
